...
הדרך לעקוף את זה היא **fallback chain של 4 מודלים** — כל אחד עם מונה quota
נפרד — שמגיעה ל-560 בקשות יומיות בסה"כ.
...
- `models` — שרשרת fallback של מודלי Gemini. ברירת מחדל:
  ```json
  ["gemini-3.5-flash", "gemini-3.0-flash", "gemini-2.5-flash", "gemini-3.1-flash-lite"]
  ```
  הסדר: מהחדש (סטטיסטית - איכותי יותר) ליציב המוכר, ואז ל-lite לכמויות.
  שלושת ה-flash הראשונים: 20 RPD כל אחד (כל אחד עם מונה quota נפרד) =
  60 בקשות איכותיות ביום. כש**שלושתם** נגמרים → fallback ל-
  ...
  לבזבז retries (זה גם חוסך RPM). רק המודל האחרון בשרשרת מקבל retry עם
  backoff (כי אין למי ליפול עליו).
